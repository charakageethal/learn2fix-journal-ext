...
def simulate_human(test_input, bug_dir, bug_prog, gold_prog):
    actual_output = run_test(test_input, bug_dir+"/"+bug_prog)
    expected_output = run_test(test_input, bug_dir+"/"+gold_prog)
    return actual_output == expected_output

def ask_human(test_input,bug_dir,bug_prog):
    formatted_input = format_input(test_input)
    process = subprocess.Popen(('echo', formatted_input), stdout=subprocess.PIPE)
    actual_output=""
    try:
        actual_output = subprocess.check_output(["timeout","-k","2s","2s", bug_dir+"/"+bug_prog], stdin=process.stdout,encoding="utf-8")
    except:
        pass

    for input_idx,t_input in enumerate(test_input):
        print("Input "+str(input_idx+1)+":"+str(t_input))

    human_readable_actual_output=subprocess.check_output(["sh",bug_dir+"/"+"convert.sh",actual_output], encoding="utf-8")
    print("Program output:"+str(human_readable_actual_output))
    human_feedback=None
    feedbacks=["Y","y","N","n"]

    while(human_feedback not in feedbacks):
        human_feedback=input("Did the program return the correct output(Y/N):")
        if human_feedback in ["Y","y"]:
            print("Correct output-Passing input:"+str(test_input))
            return True
        elif human_feedback in ["N","n"]:
...
test_labels = np.array([simulate_human(test_input, bug_dir, bug_prog, gold_prog) for test_input in test_inputs])
...
while time.time() < timeout and n_human_labeled < max_labels:
    ...
    n_generated += 1
    if False == simulate_human(fuzzed_test_case[:-1], bug_dir, bug_prog, gold_prog):
        n_failing += 1
...
heldout_test_labels = np.array([simulate_human(test_input, bug_dir, bug_prog, gold_prog) for test_input in heldout_test_inputs])
heldout_test_suite = []
