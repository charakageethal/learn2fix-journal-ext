...
def run_test_raw(test_input,program):
    formatted_input = format_input(test_input)
    process = subprocess.Popen(('echo', formatted_input), stdout=subprocess.PIPE)
    test_output=""
    try:
...
def get_noisy_output(seed_input):
    while True:
        mutated_input=custom_mutation(seed_input)
        seed_output=run_test(seed_input,bug_dir + "/" + bug_prog)
        mutated_output=run_test(mutated_input, bug_dir + "/" + bug_prog)
        if mutated_output!=seed_output:
            break

    formatted_input = format_input(mutated_input)
    process = subprocess.Popen(('echo', formatted_input), stdout=subprocess.PIPE)
    original_mutated_output=""
    try:
        original_mutated_output = subprocess.check_output(["timeout","-k","2s","2s", bug_dir+"/"+bug_prog], stdin=process.stdout,encoding="utf-8")
    except:
        pass

    return original_mutated_output
...
parser.add_argument("-n", "--percentage_incorrect_human_labels", help="percentage of incorrect human labels", default=0.1, type=float)
...
debug = args.debug
noise_level=args.percentage_incorrect_human_labels
...
classifier=CustomClassifier("DCT")
...
sequence = [i for i in range(max_labels)]
random_incorrect=sample(sequence, int(max_labels*noise_level))
...
while time.time() < timeout and n_human_labeled < max_labels:
    ...
    predict_label= classifier.predict(np.array([fuzzed_test_case]))
    is_incorrect_label=False
    ...
    if predict_label == False or (trainer_labels.size > 10 and all_fail_prob > 0.9 and predict_label == True):
        ...
        if n_human_labeled in random_incorrect:
            if debug: print("[INFO] Incorrect human label:")
            is_incorrect_label=True
            human_label = not ask_human(fuzzed_test_case[:-1], bug_dir, bug_prog, gold_prog)
        else:
        ...
        if len(oracle_committee) == 0:
            for i in range(committee_size):
                ...
                oracle_committee.append(classifier_pass)
...
predicted_labels = classifier.predict(heldout_test_suite)
...
if debug:
    ...
    print("Incorrectly labeled:"+str(random_incorrect))

    print("GENERATION")
    print("Number of labeled / generated test cases: " + str(n_human_labeled) + " / " + str(n_generated))
    print("Number of labeled / failing   test cases: " + str(n_labeled_failing) + " / " + str(n_failing))

    if n_failing > 0 and n_generated > 0 and n_human_labeled > 0:
        improvement = ( n_labeled_failing/ n_human_labeled) / (n_failing / n_generated)
...
print(bug_dir.split("/")[-1]+","+str(iteration)+","+str(n_generated)+","+str(n_human_labeled)+","+str(n_failing)+","+str(n_labeled_failing)+
...
autogenOutfiles=glob.glob(bug_dir + '/' + 'autogen-out-' + str(iteration) + '-*', recursive=False)

for f in incalFiles:
    os.remove(f)

for f in autogenOutfiles:
    os.remove(f)
...
for tl in trainer_labels:
    ...
    fout= bug_dir + "/" + "autogen-out-"+str(iteration)
    if tl == True:
        fin = fin + "-p" + str(pindex)
        fout = fout + "-p" + str(pindex)
        autogen_out=run_test_raw(cur_input, bug_dir+"/"+bug_prog)
        pindex = pindex + 1
    else:
        fin = fin + "-n" + str(nindex)
        fout =fout + "-n" + str(nindex)
        nindex = nindex + 1
        if (lindex-1) in random_incorrect:
            autogen_out=get_noisy_output(cur_input)
        else:
            autogen_out=run_test_raw(cur_input, bug_dir+"/"+gold_prog)

    with open(fin,"w+") as autogen_test_input_file: 
        autogen_test_input_file.write(format_input(cur_input)+"\n")

    with open(fout,"w+") as autogen_test_output_file:
        autogen_test_output_file.write(autogen_out)
