...
parser.add_argument("-c", "--classification_algorithm", help="classification algorithm",required=True)
...
class_algo = args.classification_algorithm
...
classifier=CustomClassifier(class_algo)
...
if debug:
    ...
    print("[INFO] Classification algorithm: " + class_algo)
    ...
    if predict_label == False or (trainer_labels.size > 10 and all_fail_prob > 0.9 and predict_label == True):
        ...
        if len(oracle_committee) == 0:
            for i in range(committee_size):
                ...
                classifier_pass=CustomClassifier(class_algo)
                classifier_fail=CustomClassifier(class_algo)
