...
rundir="$rootdir/angelix-run-$1-$2-$3"  # Directory for running the test script, a temporary needed to be created as an output directory where everything will be copied to during the repair
angelixexperimentsdir="$rootdir" # directory in which angelix is called from
log_file="$rundir/test-results-$1-$2-$3.log"  # Log file for result summary

ignore_file="$rootdir/versions-ignored-all.txt"  # File contains list of subjects to be ignored
angelix_backend_klee_out_0="$angelixexperimentsdir/.angelix/backend/$1-$2-$3-klee-out-0"
runNewSynthesizer=""  # To store if --use-nsynth option is enabled [applicable to angelix]
runVerbose="--verbose"  # Change to "--verbose" if need to run angelix in verbose
...
if [[ "$version" == *"-bug-"* ]]; then
   ...
   defect=$(cat $filename | grep $version | cut -d$'\t' -f 2)
   verdict=$(cat $filename | grep $version | cut -d$'\t' -f 3,4)
  ...
  if ! grep -q "$version" $ignore_file; then
    ...
    if [ "$3" == "manual" ]; then
      ...
      testscriptname="test-angelix.sh"
      positiveCases=($(grep -E "p[0-9]+\)" $testAngelix | cut -d')' -f 1))
      negativeCases=($(grep -E "n[0-9]+\)" $testAngelix | cut -d')' -f 1))
      
    else
      testscriptname="test-angelix-autogen-noise.sh"
      positiveCases=$(ls $BUGGY_DIR/autogen-$2-p* | rev | cut -d'/' -f 1 | rev)
      negativeCases=$(ls $BUGGY_DIR/autogen-$2-n* | rev | cut -d'/' -f 1 | rev)
    fi
    ...
    #learnfix time calculation:
    learn2fixrepairtime=0

    if [ "$timeStr1" != "----" ]; then
      ((learn2fixrepairtime+=$(echo "$timeStr1" | tr -dc '0-9')))
    fi

    if [ "$timeStr2" != "----" ]; then
      ((learn2fixrepairtime+=$(echo "$timeStr2" | tr -dc '0-9')))
    fi

    if [ "$timeStr3" != "----" ]; then
      ((learn2fixrepairtime+=$(echo "$timeStr3" | tr -dc '0-9')))
    fi

    if [ "$timeStr4" != "----" ]; then
      ((learn2fixrepairtime+=$(echo "$timeStr4" | tr -dc '0-9')))
    fi
    ...
    #Concatenate learn2fixoutput:
    learn2fixout+="$learn2fixrepairtime,"
    ...
    if [ $success -eq 1 ]; then
      ...
      learn2fixout+="$accepted,$gCount,$pCount"
